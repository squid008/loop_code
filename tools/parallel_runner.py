    ...
    try:
        ...
        while True:
            ...
            if stopped_by_user:
                ...
                #   收尾条件 = 「没有任何池在跑」+「**本轮有过真实进展**」✓ —— 若本轮一代都没跑完
                #   （在跑的都被停止杀掉 ⇒ 该代作废、没写任何东西 ✓），就**没有可审的东西** ⇒
                #   收尾（全库体检 + 跨池审查 + 精选池 + 指标/曲线）纯属**白跑 ~10 分钟** ✗ ⇒ 跳过才对 ✓
                #   ⚠ 但"跳过"必须**有声音**：否则用户以为"点了停止却没审查" ✗（实测疑问 ✓）
                if not ran_round:
                    RT.log('  ★ 本轮**没有任何一代跑完**（在跑的都被停止 ⇒ 该代作废）')
                    RT.log('     ⇒ 没有新进展可审，**跳过收尾**（不白跑一遍全库体检 ✓）')
