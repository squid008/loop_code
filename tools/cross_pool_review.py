...
def main():
    ...
    L += ['## ⚠ 下游使用须知（**先读这段再用**）', '',
          '1. **必须乘 `sign`**（见「精选因子明细」）：引擎求值时对 `sign<0` 的候选**取负**；'
          '不乘 ⇒ **方向反了、组合反向选股**。',
          '2. ★★★ **本表的「剥风格」是「评估口径」，不是 `facs/` 里的值** —— '
          '`facs/<xx>/<name>/values.h5` 存的是**原始因子值**（未经中性化）。',
          '   - 剥风格的定义 = **把因子对 `lncap` + `lnamt` 做截面秩中性化后重跑回测**'
          '（引擎 `loop_engine.py` 的 `--strip_style`）。',
          '   - ⇒ **要做到本表的绩效，下游必须自己实现这一步**（取残差后再排序）；'
          '直接用原始值 ⇒ **绩效与风险回撤都对不上**（尤其 `F33`：未剥日频回撤 ≈ −19%，'
          '剥后仅 −10%）。',
          '   - ⚠ **不能用 `values_q.h5`（uint8 快查副本）做这件事** —— '
          '它**丢了原始值**（只保留截面秩）⇒ 只能做秩类运算，**做不了中性化/回归**。',
          '3. **本表未含「池内（300/500/1000）」结论**：这 7 个因子**基本从未在池内测过**'
          '（`--pool_obs` 是 2026-09-12 才加的，而它们更早入库）⇒ 池内表现需**另测**。',
          '   （已知的外部实测结论：这 7 个在 **300 内全员失效**、500 内仅 `F07` 勉强 '
          '⇒ **不要搬进成分内**。）', '']
