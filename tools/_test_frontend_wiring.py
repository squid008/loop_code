...
print()
print('【10】★★★★★ 「全部停止」到底会不会收尾（2026-09-23 用户实测："我点全部停止好像没有看到'
      '审查过程？只看到并行中？"）')
# 真相：收尾 = 「没有池在跑」+「**本轮有代跑完**」⇒ 一代都没跑完时**跳过是对的**（不白跑全库体检 ✓），
#   但过去 UI 写的是**无条件**"然后自动做收尾审查再退出" ✗ ⇒ 用户合理预期落空 ✗✗
chk('前端 tooltip 已改成"**若本轮有代跑完**才收尾"（不再无条件承诺 ✗）',
    '若本轮有代跑完' in src and '然后自动做收尾审查再退出' not in src)
chk('★ 「全部停止」的 toast 带上后端的说明（`r.note`）⇒ 用户当场能看到为什么没收尾 ✓',
    re.search(r'!pool && r\.note \? `。\$\{r\.note\}`', src) is not None)
chk('注释也同步更正（不再写"「全部停止」后也会自动收尾" ✗）',
