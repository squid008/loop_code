...
================================================================
三策略固定等权(小市值1/3 + 白马1/3 + 红利低波1/3) + 估值择时 + 恐慌期加杠杆

历史结果(2014-10-01 ~ 2026-08-05):
  无杠杆      年化 17.71%   回撤 -31.28%   Sharpe 1.041
  +恐慌杠杆   年化 18.16%   回撤 -29.46%   Sharpe 1.050
  (基准沪深300 年化 5.58%)

与 all01.py 的区别:
  all01 额外启用"成交额拥挤度动态调权" -> 年化19.38% 回撤-28.64% Sharpe1.088
  本文件为对照基准, 三策略权重固定不调, 便于对比调权的增量
...
# ---- 小市值选股的排序方式(因子挖掘结论) ----
# 单因子检验(2018-2026, 池内等权, 5日换仓):
#   小市值 ln_mktcap  超额 +6.88%  <- 最强单因子
#   小成交额 amt_log10 超额 +3.82%
#   两者相关, 故按权重合成; 市值中性化后其余量价因子均无独立增量(93个全部未通过)
SMALL_USE_AMT = True     # True=总市值+成交额双因子, False=沿用流通市值单因子
SMALL_W_MC = 0.60        # 总市值权重
SMALL_W_AMT = 0.40       # 成交额权重(对数排名)
...
# all00=固定等权基准(关闭), all01=仅调权, all03=双因子+调权(最优, 默认开启)
USE_CROWD_W = True
...
def pick_small(context, bar_dict):
    ...
    if SMALL_USE_AMT and 'mktcap' in df.columns and 'volume' in df.columns:
        # 双因子: 小市值(总市值) + 小成交额, 各自截面排名后加权
...
if __name__ == '__main__':
    for a in sys.argv[1:]:
        if a.startswith('--tech='):        # 卫星仓权重, 如 --tech=0.15
            ...
            N_TECH = int(a[8:])
        elif a == '--crowd':               # 启用拥挤度动态调权(all01已验证+1.22pp)
            USE_CROWD_W = True
        elif a.startswith('--wamt='):      # 成交额权重, 默认0.40
            SMALL_W_AMT = float(a[7:])
            SMALL_W_MC = 1.0 - SMALL_W_AMT
