...
# ---- 撮合（成交量限制）----
volume_limit = True                     # 每 bar 累计成交不超过市场总成交量的 volume_percent 比例；False 可对齐聚宽全成交
volume_percent = 0.25                   # 可成交数量占市场总成交量比值，仅在 volume_limit=True 时生效,1跟聚宽对齐,0.25是米筐默认
...
def init(context):
    ...
    # 智能版 order_target_portfolio_smart 仅允许在 handle_bar（ON_BAR）阶段调用，
    # scheduler 回调（SCHEDULED）会被禁止。故改为在 handle_bar 中按"自然周首次交易日"
    # 触发，效果等价于原来的 scheduler.run_weekly(rebalance, tradingday=1)。
    context.last_week = None
    context.select_num = select_num

def handle_bar(context, bar_dict):
    # 每周第一个交易日调仓（周一是节假日时顺延到本周第一个交易日）
    week = context.now.isocalendar()[:2]
...
def rebalance(context, bar_dict):
    ...
    # ============================ 调仓 ============================
    # 用智能版 order_target_portfolio_smart（RQAlpha v6 推荐，行为对齐聚宽）：
    #   1. 不在目标组合里的旧持仓自动先卖后买，无需手动构造清仓列表；
    #   2. 停牌/无行情持仓不强行平仓（上下界钉在当前权重），避免普通版 OTP
    #      强制平停牌股 → 卖单被拒 → 市值仍占资金 → 后续买单被静默缩为 0 的"持仓 0"问题；
    #   3. 涨停限制买入、跌停限制卖出，与聚宽一致；
    #   4. 资金不足时改用剩余现金下单并打印 WARN，而不是静默缩单。
    order_target_portfolio_smart(target_stocks)
