...
def rebalance(context, bar_dict):
    ...
    # ===================== 聚宽风格调仓：先卖后买，逐只下单 =====================
    # 与聚宽 order_target 行为对齐：
    #   1. 先卖掉所有不在目标组合里的旧持仓（含停牌股——卖不掉会拒单，但只影响这一单）；
    #   2. 再逐只买入目标股，资金不足时该单截断到剩余现金（不影响其他单），
    #      避免普通版 order_target_portfolio 那种"共享资金池连续缩单 → 全局重排"
    #      导致的参数路径敏感问题。
    for p in get_positions():
        if p.order_book_id not in target_stocks:
