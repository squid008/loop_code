...
> **当前版本 `v1.3.3`**（2026-09-16）— ★★ 修「点一键启动全部没反应」：**`window.confirm()` 在 IDE 内置浏览器里会被阻止**（不弹窗、直接返回 false）⇒ `if (!confirm(...)) return` 变成"点了毫无反应" ✗（实测后端日志里连 `POST /api/mine/start` 都没有）⇒ **改用自定义确认弹窗**（React 组件，任何环境可用；三处 `doStart`/`doStop`/`doStartPool` 全替换）；
