...
> **当前版本 `v0.15.0`**（2026-09-15）— 池化挖掘闭环 + B角「诊断→决策」链条修复 + 收尾提速 + 管线迁入 `tools/` + **修 `--pool_gate_or_all` 静默绕过硬门槛** + **算子族扩展（`ema` + 回归三件套 + 偏度/峰度，`UNARY` 40→53）**
...
- `tools/` — **生产管线与工具（纳入 git）**。2026-09-14（v0.5.2）从 `ai_test/` 迁出：
  ...
  - `_test_critic_sensor.py`（32 项）· `_test_build_facs_merge.py`（11 项）· `_test_inject_pools.py`（17 项）
    ...
    · `_test_ops_sync.py`（22 项，**算子表单事实源 + 防副本漂移**）
    · `_test_ops_math.py`（44 项，**新算子数值对拍**：`np.polyfit` / `scipy.stats`）回归测试
    · `_check_quotes.py` 质检
