...
> **当前版本 `v0.9.1`**（2026-09-14）— 池化挖掘闭环 + B角「诊断→决策」链条修复 + 收尾提速 + 管线迁入 `tools/` + **修 `--pool_gate_or_all` 静默绕过硬门槛**
...
- `tools/` — **生产管线与工具（纳入 git）**。2026-09-14（v0.5.2）从 `ai_test/` 迁出：
  ...
  - `_test_critic_sensor.py`（32 项）· `_test_build_facs_merge.py`（11 项）· `_test_inject_pools.py`（17 项）
    · `_test_parent_sel.py`（20 项）· `_test_reports_expr.py`（27 项，**报告表达式必须完整**）回归测试
    · `_check_quotes.py` 质检
...
| tag | 日期 | 提交 | 一句话内容 |
|---|---|---|---|
| **v0.9.1** | 2026-09-14 | `tag 自身` | **修 `factor_pool_selected.md` 的表达式被截断**（`[:58]` ⇒ 最长 **58→189**）—— 根因：该文件**只有表格、没有明细段**（对比 `factor_library*.md` 是「总览截断 + 明细全文」两层）⇒ **截了就无处可查** ✗ · ★ **顺带补上 `sign`（方向，必须乘）与 h5 路径** —— 原来 7 个精选因子里 **6 个 `sign=-1`**，不乘它**组合会反向选股** ✗ · 新增 `tools/_test_reports_expr.py`（27 项，编码"没明细段的文件不许省略"）+ **`tools/verify_test_catches.py`**（**负向验证**：把 bug 放回去看测试是否 FAIL —— 已实测抓得到）|
