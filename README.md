...
> **当前版本 `v1.3.4`**（2026-09-16）— ★★ 修「启动按钮不变灰」+ **全部后台静默（不弹黑窗）**：① 按钮改为按「**是否在运行**」判定（`running` ⇒ 启动变灰显示"已在运行"、停止可点；反之亦然）✗ 原来只看"请求中"⇒ 请求完就复位、按钮一直可点；② ★★ **弹窗根因是 Windows"控制台继承"陷阱**：看板用 `DETACHED_PROCESS` 起调度器 ⇒ 无控制台 ⇒ 调度器再起引擎时 Windows **新建控制台窗口** ✗；⇒ 统一改用 **`CREATE_NO_WINDOW`**（**隐藏但可继承**，优于 `DETACHED_PROCESS`）；**主链路 12/12 处全覆盖**（含看板每 10 秒查进程的 `pools.py` —— 不加会一直闪黑窗）；
