...
> **当前版本 `v1.3.6`**（2026-09-16）— ★★★ 修「**停了全A却还在跑 / 显示 5 池 / 两个按钮都能按 / 下一个池不开始**」（一个根因引出四个症状）：`run_tracks.py` 的 `extra` 里**硬编码了 `--pools=300,500,1000`**（那是 `--pool_obs` 的配套功能参数，"额外算这几个池的指标"，**不代表它在跑那几个池**）⇒ 而老 `classify_proc` "没有 `--mine_pool` 就退到 `--pools`" ⇒ **`all` 的引擎被误判成在跑 300/500/1000** ✗ ⇒ 于是：`stop(all)` **找不到引擎杀不掉** · `byPool` **一个 PID 算 3 个池**（"在跑 5 池"）· `mining` 与 `engine` **不一致**（两按钮都亮）· 引擎没死 ⇒ **调度器等它、下一个池不开始** ✗✗；**修 4 处**：① `run_tracks.py` **所有池都传 `--mine_pool`（含 `all`，幂等）** · ② `pools.py` **归属按 `kind` 分开解析（engine 只认 `--mine_pool`）** · ③ `mine.py` `mining` 改为"真的有引擎在跑"（与 `engine` 一致）· ④ **`stop(pool)` 兜底**：`ctl.curPool == pool` 时把当前引擎一并杀（不依赖解析）✓；
