...
> **当前版本 `v1.3.1`**（2026-09-16）— ★ 修 `v1.3.0` 的 **UI 自相矛盾**：前端把「**配置意图**」与「**此刻是否在跑**」揉成一个布尔值 ⇒ 调度器停止后卡片仍谎报「已参与轮转」、且「启动本池」被禁用（用户报"顶部写空闲、卡片写已参与轮转"）✗ ⇒ 拆成 `configured`（已配置）与 `inRotation`（在跑），新增「待启动」态 + 按钮可点（会自动重启调度器）；
