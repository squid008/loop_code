...
> **当前版本 `v0.5.3`**（2026-09-14）— 池化挖掘闭环 + B角「诊断→决策」链条修复 + 收尾提速 + 管线迁入 `tools/` + **修 `--pool_gate_or_all` 静默绕过硬门槛**
...
| tag | 日期 | 提交 | 一句话内容 |
|---|---|---|---|
| **v0.5.3** | 2026-09-14 | `tag 自身` | 🔴 **修 `--pool_gate_or_all` 静默绕过「剥风格门槛 + 分段验证」**（`combine_ok()` 纯函数 + `_ok_hard`；`tools/_test_ok_gate.py` 22 项）· `tools/prune_style_only.py`（1000 池 bank 9→3）· 删 3 个垫片 · **产品定位拍板 B. 真中性增强** |
