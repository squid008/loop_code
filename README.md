...
> **当前版本 `v1.3.7`**（2026-09-16）— ★★★ 修「**点了停止全A，顶上「在跑的池」还是 5/5**」：`pool_status` 有两处错 ① `not pr["pools"]` 宽兜底（解析不到归属 ⇒ 就当它跑遍所有池）✗ ② **`driver`（调度器）也算** —— 它的命令行是 `--pools=all,300,500,1000,50` ⇒ **5 个池全命中** ✗（但那只是"它**管**这 5 个池"，**不代表此刻都在跑**）⇒ 两者叠加 ⇒ **永远 5/5** ✗；**修**：**"在跑的池" 只认「该池真的有 engine 在跑」**（去掉宽兜底 + 去掉 driver 参与）✓；
