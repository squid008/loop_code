...
> **当前版本 `v1.2.0`**（2026-09-16）— ★★★ **看板改「每池一个独立进程」**：可**真正单独启停任意池**、池间**互不干扰**（旧版是 1 个串行驱动 ⇒ 实测 9.3 小时全被 300 吃光、500/1000/50 一代没跑 ✗；且"停某个池"不可能）；新增 `run_tracks.py --no_global` 挡掉唯一的全局冲突点 + 启动前**内存检查**（实测 27 GB 可用 ⇒ 最多并行 3 个）+ 修「停止按钮没反应」（不再用 disabled 阻断，点了必给反馈）；
