...
> **当前版本 `v0.14.0`**（2026-09-15）— 池化挖掘闭环 + B角「诊断→决策」链条修复 + 收尾提速 + 管线迁入 `tools/` + **修 `--pool_gate_or_all` 静默绕过硬门槛** + **算子族扩展（`ema` + 回归三件套 + 偏度/峰度，`UNARY` 40→53）**
...
| tag | 日期 | 提交 | 一句话内容 |
|---|---|---|---|
| **v0.14.0** | 2026-09-15 | `tag 自身` | **§1.3-B 核查：「生成后、L1 前」那道门在本项目没有可用判据**（用 `bank` vs `last_l1` 未入库 做正负样本标定：`length`/`n_nodes`/`n_fields` 作上界**只能拦 0~5%**；结构 Novelty **正负中位重合**）—— 根因：**`cfg['depth']` 已在生成端管住复杂度**（QuantaAlpha 需要它是因为候选由 **LLM 写**）；且我们表达式表示不同（`ts_mean20(close)` vs `TS_MEAN($close,20)`）⇒ **照抄 `unique_vars_ratio>=0.5` 会误杀 21.7% 已入库因子** · ★★★★★ **真正的发现**：预算瓶颈在 **L2**（`n_l1=28→n_l2=28→n_pass=2`，命中率 4.8%），而那里**已有一整套门**，其中 **`--min_mono` 已标定（1150 条：零误杀拦 27%）却默认关着** —— 我用**独立数据**（124 条）复核得 **35.6%**，两次独立标定互相印证 · **新增 `tools/calib_gate.py`（加门前先跑它）** · ⚠⚠ **更正**：早先报的"Novelty 方向反了"是**实现 bug**（`zip` 两列表错位），真相是**无区分度** · ★★★ 教训：**并行列表绝不要靠两个同条件推导式对齐**（我同一次任务犯了两次）· **未改引擎行为** |
