...
> **当前版本 `v0.7.0`**（2026-09-14）— 池化挖掘闭环 + B角「诊断→决策」链条修复 + 收尾提速 + 管线迁入 `tools/` + **修 `--pool_gate_or_all` 静默绕过硬门槛**
...
| tag | 日期 | 提交 | 一句话内容 |
|---|---|---|---|
| **v0.7.0** | 2026-09-14 | `tag 自身` | **第一批改造（判据与口径族）**：`strip_grade` 改**日频** + 新增 **A 档回撤上限 `dd_d > -0.20`**（回撤超限**只降 B 不判 C**）· 新增 `TAG_POOL_FLOOR_CAL=0.15` **修判据不对称**（原来池内只要超额>0 ⇒ `all3` 名不副实）· L2 三处回测开 `with_daily` · `loop_pool_obs` 加 `pools_scope`（§1.5）· **验证：`F10_1000` 标签 `all3`→`csi1000_only`，与外部审查闭环** |
