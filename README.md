...
> **当前版本 `v1.3.0`**（2026-09-16）— ★★★ **挖掘改「单调度器 + 池轮转」**：内存从"每池一份"降到 **1/5**（只读面板 `B` 实测 **3.25 GB/进程** ⇒ 5 池要 30 GB > 可用 27 GB，**根本起不来** ✗；轮转后同时只 1 个引擎 ⇒ ~6 GB ✓，且 CPU 仅 6 核而单引擎已吃满多核 ⇒ 并行损失≈0）；★ **每轮结束自动收尾** + **一键全部停 ⇒ 自动收尾**（不用再手点，按钮已隐藏）；★ 启停任意池**不影响收尾**（收尾条件是"无池在跑"+“本轮有进展”，不是"所有池跑完"✗）；实修 **5 个真 BUG**（停池误清 enabled / 恢复报 409 / stopAll 残留阻塞 / 全部停无人收尾 / phase 卡 tail）；前端加**顶部状态条**（空闲·挖掘中·收尾审查中）；
