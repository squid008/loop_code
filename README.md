...
> **当前版本 `v1.3.8`**（2026-09-16）— ★★ 池卡片「停止本池」改为**随时可剔除**（用户报「已参与轮转还未轮到的，我也要可以停止那个池吧？不然我得**挨个点停止才会轮到我想停的池子**」）：v1.3.5 把"停止"误解为"只停当前那一代"⇒ 「轮转中/待启动」（**还没轮到**）时按钮是灰的 ✗；⇒ 修正为 **`disabled = busy || !configured`**（只要"在轮转里"就可点）✓
