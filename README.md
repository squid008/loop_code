...
> **当前版本 `v0.16.0`**（2026-09-15）— 池化挖掘闭环 + B角「诊断→决策」链条修复 + 收尾提速 + 管线迁入 `tools/` + **修 `--pool_gate_or_all` 静默绕过硬门槛** + **算子族扩展（`ema` + 回归三件套 + 偏度/峰度，`UNARY` 40→**56**）** + **架构清扫 P0-1：算子「单一事实源」**（加 1 个算子 **10 处 → 1 处**；★ 顺带修掉「**生效版 A角 prompt 缺 21 个算子 + 7 个叶子**」的真 bug）
