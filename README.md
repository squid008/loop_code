...
> **当前版本 `v1.1.0`**（2026-09-16）— ★ **看板加上「启动 / 停止挖掘」按钮**（每个池卡片「启动本池/停止」+ 顶栏「一键启动/全部停止」，都在「立即刷新」左边）；★★ **核心安全：防重复启动**（已有驱动/引擎在跑 ⇒ **409 拒绝**，避免两份驱动抢 `loop_state*.pkl` **损坏因子库**）+ 池名白名单 + 轮数范围校验 + 子进程脱离父进程；
