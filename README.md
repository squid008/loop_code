...
> **当前版本 `v1.0.3`**（2026-09-15）— ★★★ **修「`del f` 后又用 `f`」导致**每代必崩**的真 BUG**（v0.20.0 修 `e` 时漏掉的第二个幽灵：`run()` 内 L2519 `del f` 释放候选面板，而循环外 L2663 的 `_save_state(..., f, ...)` 还在用它 ⇒ `UnboundLocalError`；且该形参本就是幽灵 —— 函数体内 `with open(..) as f:` 把它局部重绑定了）+ **新增防线 `tools/_test_del_reuse.py`**（已负向验证）+ 重启 5 池×50 轮；
