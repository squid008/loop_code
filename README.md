...
> **当前版本 `v1.3.2`**（2026-09-16）— ★ **前端所有鼠标提示改成自然语言**（用户报「你的所有鼠标提示里面的引号都改改，**看着就是 AI 输出的**」）：`title` / `confirm` / `toast` 都是**纯文本**、浏览器**不渲染 Markdown** ⇒ `**加粗**` 会原样显示、再加上满屏 `★ ⇒ ✓` ⇒ 一眼机器味 ✗；⇒ 7 处 tooltip + 3 处确认框 + 4 处 toast + 2 处页面文字全部改为自然句子（去掉 `**` 与"工程符号"）；新增常驻检查 `tools/_chk_ai_tone.py`（扫用户可见区，**实测残留 0**）；
