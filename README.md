...
> **当前版本 `v0.6.1`**（2026-09-14）— 池化挖掘闭环 + B角「诊断→决策」链条修复 + 收尾提速 + 管线迁入 `tools/` + **修 `--pool_gate_or_all` 静默绕过硬门槛**
...
| tag | 日期 | 提交 | 一句话内容 |
|---|---|---|---|
| **v0.6.1** | 2026-09-14 | `tag 自身` | **PATCH**：全库 50 因子日频实测后**更正 v0.6.0 的过度陈述** —— 「期频**系统性**低估 3.6pp」是 `F10` 极端个案，**全库折算比中位 0.928**（回撤平均只放大 ~1.08×）⇒ 日频的价值在**揪出个别高回撤因子**（`F03_1000` 日 dd **−35%**、`F11` **−23%**）· 新增 `tools/calib_daily_threshold.py`（**阈值敏感度表**：日频 0.25 ⇒ A 档 17 个 **与现状相同**）|
