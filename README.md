...
> **当前版本 `v1.3.9`**（2026-09-16）— ★★★ 修「**点 2 个池停止，再启动另一个池 ⇒ 其它池的剔除全被取消**」（用户报「我点了2个池子停止，然后重新启动中证500池，怎么其它池子都给取消了？」）：**根因 = 4 处代码都在清 `stopped`**（`mine.start()` 两处 · `mine.stop(None)` · **`run_tracks.py` 调度器退出复位**）—— ★ 但 **`stopped` 是「用户的配置」、不是运行时状态** ✗；**修**：明确「配置 vs 运行时」边界 ⇒ `start()` 新增 `reset_stopped`（只有「一键启动全部」才清）· `stop(None)` 只设 `stopAll` · 调度器退出**不清 `stopped`** ✓；前端按钮规则定稿（两按钮**互补无重叠**）+ 新增「**运行中·已移出**」状态 ✓
