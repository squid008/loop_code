...
| tag | 日期 | 提交 | 一句话内容 |
|---|---|---|---|
| **v0.13.0** | 2026-09-15 | `tag 自身` | **算子族扩展 ②③：回归三件套 + 偏度/峰度**（+13 个算子，`UNARY` 40→**53**）—— 用户点名 **slope + R²**：`ts_slope{5,20,60}`（趋势方向+强度）· `ts_rsqr{5,20,60}`（趋势**干净度**）· `ts_resi{5,20,60}`（同一次回归 ⇒ 近零额外成本）+ `ts_skew{20,60}`/`ts_kurt{20,60}`（**分布形状**，与波动率不等价）· ★★ **回答「R² 高说明涨得稳，你觉得呢」：方向对，但补三点** —— ① **R² 不含方向**（涨得稳/跌得稳 R² 相同，已固定成测试 ⇒ 必须与 `ts_slope` 组合）② 价格不动时 **0/0 ⇒ 输出 NaN**（不臆造、也不给假 1.0）③ **尺度无关** ⇒ 捕捉的是「路径有无单边趋势」而非「涨得多不多」· ★★ **「成本高的」用实测定义**（前缀和型 0.42s / 排序型 10× / **位置型 164×**）⇒ 顺带发现**「位置」可用「幅度差」替代**（`sub(ts_max100(x), x)` 能组合）· ★ **数学类不做**（`sqrt`/`pow`/`exp`：**对秩类因子无效**）· 对拍 **`np.polyfit`/`scipy.stats` 逐位一致** + 两个负向验证 + 真机冒烟 5/5 · ⚠⚠⚠ **事故与修复**：**并行跑两个负向验证把源码改坏**（进程②把①的错公式"还原"回去）⇒ 给 `verify_test_catches.py` 加**并发守卫 + 还原后逐字节校验** |
