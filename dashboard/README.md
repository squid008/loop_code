...
## 〇、★★★ 版本也只改一个地方（根 `VERSION`）

```
VERSION                      ← ★ 项目版本唯一来源（仓库根，内容如 `1.0.0`）
  ├── README.md              ← 顶部「当前版本」必须等于它
  ├── change_log.md          ← 最新 `## [x.y.z]` 条目必须等于它
  ├── api/app/settings.py    ← **读**它（看板 `/api/meta` 的 version 就来自这里）
  └── web/package.json       ← version 字段必须等于它
       └── 看板页脚显示「项目版本 v1.0.0」★ 与实际版本一致
```

> **为什么要这样**（2026-09-15 真实事故）：版本号此前散落三处，实测**三处打架** ✗
> | 位置 | 原值 | 差距 |
> |---|---|---|
> | git tag 最新（真实） | `v0.21.1` | — |
> | `change_log.md` 最新条目 | `0.20.2` | ⚠ **落后 9 版** |
> | `README.md` 顶部声明 | `v0.20.0` | ⚠ **落后 11 版** |
> ⇒ 改为**根 `VERSION` 一处**，并由 **`tools/_test_version_sync.py`** 守门（不一致即报错）✓
...
> ★ **历史快照 · 2026-09-15**（**举例说明"三个数会不一样"用，不是现状** ✗）：
> `300` → 3 / 2 / 2 · `500` → 5 / 3 / 3 · `1000` → **10 / 4 / 5**
> ⇒ 看板一律以 **`当前库`（state.bank）** 为准，并把另两个作为口径说明一并给出 ✓
> ⚠ **要看"现在各是多少"别读这一行** —— 现查：`python tools/library_kpi.py --pool=<池>`
> （秒级、直接读 state ✓）或看板「池运行状态」卡片；全库现状的口径快照在 `docs/loop_todo.md §0` ✓
> （★ 2026-09-23 文档体检：这一行原写作"实测（2026-09-15）"，容易被当成现状 ⇒ 现**显式标注为历史快照** ✓）
