      ...
      {status && (
        <section className="summary">
          ...
          <div className={`kpi phasecard t-${mine?.phase === 'mine' ? 'ok'
            ...
            //   ⇒ 这里把口径说明白 ✓，并补一行**本轮已完成代数合计**（进度就该看这个 ✓）
            (mine?.roundText
              ? `${mine.roundText}（全局轮次：每个启用的池都完成至少 3 代才加一`
                // ★ 2026-09-20（用户："啥叫三池都收齐才+1？就是三池都收入了1个因子才加1？"）
                //   ⇒ **不是"入库因子"** ✗ —— 见 `tools/parallel_runner.py` 的原文：
                //     「**直到每个启用池都完成 >= min_gens_per_round 代（默认 3）才收轮**」
                //     ⇒ 与"入没入库、入几个"无关 ✓（只数**代数** ✓）
                //     ★ 2026-09-20（方案 B）：原判据是"1 代"，但它配合重排队条件
                //       ⇒ **轮永远收不了口** ✗ ⇒ 改为"每池 >= 3 代" ✓
                + `；不限模式下快池会一直接着领下一代，所以它总是落后于实际进度）`
                + ` · 轮次上限 ${mine?.rounds ?? '?'}\n`
              : '') +
            (mine?.gensRound
              ? `本轮已完成代数合计：${Object.keys(mine.gensRound)
                  .reduce((a, k) => a + (mine.gensRound?.[k] ?? 0), 0)} 代（逐池明细见下）\n`
              : '') +
