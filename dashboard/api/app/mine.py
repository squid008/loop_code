...
def stop(pool=None, **kw):
    """停止：`pool=None` ⇒ **全部停**；指定池 ⇒ **只停该池** ✓

    ⚠ 2026-09-23 更正（用户实测疑问："点全部停止好像没有看到审查过程？"）：**全部停之后并不总是收尾** ✗ ——
      收尾的条件是「没有任何池在跑」**且**「**本轮有过真实进展**」（至少一代跑完 ✓，见 `parallel_runner`
      的 `ran_round` / `run_tracks` 的 `dirty` ✓）。若在跑的几代都是被"停止"**中途杀掉**（该代作废、
      没写任何东西 ✓），就**没有可审的东西** ⇒ 跳过收尾（否则白跑 ~10 分钟全库体检 ✗）✓
    """
    ...
    if sched:
        ...
        # ★★★★★ 2026-09-23（用户："我刚才点全部停止好像没有看到审查过程？只看到并行中？我看错了？"）：
        #   用户没看错 —— 那次**确实没收尾** ✓。真行为（`parallel_runner` / `run_tracks` 的收尾条件）：
        #     **没有任何池在跑** ＋ **本轮有过真实进展**（至少一代**跑完** ✓）
        #   ⇒ 在跑的几代若都是"被停止中途杀掉"（该代作废、**没写任何东西** ✓，引擎是代末原子写 ✓），
        #     那就**没有新东西可审** ⇒ 收尾（facs 落地 + 跨池审查 + 精选池 + 指标表 + 曲线）纯属
        #     **白跑 ~10 分钟** ✗ ⇒ 正确行为是**跳过** ✓
        #   ⚠ 但过去这里的措辞是**无条件**收尾承诺 ✗✗（前端 tooltip 与那行注释同样 ✗）
        #     ⚠⚠ 本注释**故意不照抄那句老话** —— 静态守门 `_test_sched_kgen` 断言它"不该再出现" ✗，
        #       照抄进注释会被**自己的注释**绊倒（本项目踩过两次了 ✓ v1.21.43 / v1.21.44 ✓）
        #     ⇒ 用户合理预期"点停止 = 一定看到审查" ✗ ⇒ 现在**把条件说清** ✓（如实说"什么时候才收尾" ✓）
        #   ⚠⚠ 本串会**原样显示在看板上**（前端 toast）⇒ 一律自然语言 + 普通标点
        #     （不许 `**` / 反引号 / ASCII 引号 / ★⇒✓✗ 这类机味符号 —— 守门 `_test_ui_quotes` ✗）
        note = ('已请求全部停止：当前那一代已结束（作废、下次重跑）。'
                '若本轮有代跑完（有新进展），调度器会先自动收尾（facs 落地 + 跨池审查 + 精选池）'
                '再退出；一代都没跑完就直接退出，不做无谓的收尾（没有可审的东西）。'
                '想现在就体检一次：命令行跑 python tools/run_tracks.py --rounds=0')
    else:
