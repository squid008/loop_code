...
def library(pool):
    ...
    for f in factors:
        ...
        # ★★★★★ 2026-09-22（用户："因子库加一列**卡玛**的展示" + "**注意是卡玛，不是剥后卡玛，
        #   也不是超额卡玛**"）—— ⇒ 取**组合自身**卡玛（Top10% 等权的**绝对**口径 ✓
        #   = 指标表 `calmar_top` ✓）。三者在明细页都有、且**互不相同**：
        #     · 超额卡玛 `calmar` ✗（组合 − 基准 ✓）· 剥后 `strip_calmar` ✗（剥 lncap+lnamt ✓）
        #     · **组合自身 `calmar_top` ✓** ← 列表这一列要的就是它 ✓
        f['cal_top'] = (f['metrics'] or {}).get('calmar_top')
