...
|---|---|---|---|---|---|---|
| 1 | `F07` | **A** | **0.924** | +5.3% | 0.820 | `ts_std100(mul(cs_demean(hl_ratio), ts_mean5(cs_demean(turn_ratio))))` |
| 2 | `F04_300` | **A** | **0.564** | +5.1% | 0.727 | `ts_mean10(ema60(corr200(fa_sell_exp, cs_rank(ema12(cs_rank(ema12(ema12(mktcap))))))))` |
| 3 | `F35` | **A** | **0.561** | +2.6% | 1.077 | `mul(ts_max20(max(max(barra_residual_volatility, div(barra_residual_volatility, barra_liquidity)), ts_rank60(cs_scale(ts_std100(barra_residual_volatility))))), corr100(true_range, mf_m_buy))` |
| 4 | `F08_500` | **A** | **0.546** | +3.9% | 0.521 | `ts_mean200(ema20(ts_rank200(overnight)))` |
| 5 | `F25` | **A** | **0.537** | +3.9% | 1.295 | `max(ts_mean150(corr100(ts_rank100(cs_rank(barra_leverage)), ts_min20(cs_rank(barra_leverage)))), barra_residual_volatility)` |
| 6 | `F07_1000` | **A** | **0.464** | +6.3% | 0.506 | `ts_mean200(mul(add(sub(overnight, barra_beta), ts_mean60(barra_residual_volatility)), ts_max20(ts_mean60(barra_size))))` |
| 7 | `F33` | **A** | **0.373** | +3.9% | 0.847 | `ts_delta120(add(ts_max20(ts_max100(barra_non_linear_size)), div(cs_rank(barra_beta), ts_std150(mf_x_bqty))))` |
| 8 | `F05_500` | **A** | **0.332** | +2.3% | 0.802 | `corr100(mul(mf_m_sqty, ts_std60(mf_x_buy)), mf_m_sqty)` |
| 9 | `F21` | **A** | **0.305** | +2.2% | 0.868 | `max(add(barra_residual_volatility, barra_leverage), max(ts_min20(cs_rank(div(fa_ocf_yoy, fa_gm))), barra_residual_volatility))` |
...
| 剥风格判定 | **A 独立有效**（剥掉 lncap+lnamt 后**日频** Calmar 0.924 ≥ 0.30）|
| 剥风格后 Calmar（日频）/ 超额 | **0.924** / +5.3% |
...
| 剥风格判定 | **A 独立有效**（剥掉 lncap+lnamt 后**日频** Calmar 0.564 ≥ 0.30）|
| 剥风格后 Calmar（日频）/ 超额 | **0.564** / +5.1% |
...
| 剥风格判定 | **A 独立有效**（剥掉 lncap+lnamt 后**日频** Calmar 0.561 ≥ 0.30）|
| 剥风格后 Calmar（日频）/ 超额 | **0.561** / +2.6% |
...
| 剥风格判定 | **A 独立有效**（剥掉 lncap+lnamt 后**日频** Calmar 0.546 ≥ 0.30）|
| 剥风格后 Calmar（日频）/ 超额 | **0.546** / +3.9% |
...
| 剥风格判定 | **A 独立有效**（剥掉 lncap+lnamt 后**日频** Calmar 0.537 ≥ 0.30）|
| 剥风格后 Calmar（日频）/ 超额 | **0.537** / +3.9% |
...
| 剥风格判定 | **A 独立有效**（剥掉 lncap+lnamt 后**日频** Calmar 0.464 ≥ 0.30）|
| 剥风格后 Calmar（日频）/ 超额 | **0.464** / +6.3% |
...
| 剥风格判定 | **A 独立有效**（剥掉 lncap+lnamt 后**日频** Calmar 0.373 ≥ 0.30）|
| 剥风格后 Calmar（日频）/ 超额 | **0.373** / +3.9% |
...
| 剥风格判定 | **A 独立有效**（剥掉 lncap+lnamt 后**日频** Calmar 0.332 ≥ 0.30）|
| 剥风格后 Calmar（日频）/ 超额 | **0.332** / +2.3% |
...
| 剥风格判定 | **A 独立有效**（剥掉 lncap+lnamt 后**日频** Calmar 0.305 ≥ 0.30）|
| 剥风格后 Calmar（日频）/ 超额 | **0.305** / +2.2% |
...
| 被淘汰 | 与谁相关 ≥0.70 | 保留者 | 保留者剥Calmar(日频) | 被淘汰者剥Calmar(日频) |
|---|---|---|---|---|
| `F04_500` | （同组） | `F07` | **0.924** | 0.618 |
| `F05` | 0.887 | `F07` | **0.924** | 0.546 |
| `F08` | 0.899 | `F07` | **0.924** | 0.588 |
| `F10_1000` | 0.719 | `F07` | **0.924** | 0.435 |
| `F17` | （同组） | `F07` | **0.924** | 0.532 |
| `F18` | （同组） | `F07` | **0.924** | 0.416 |
| `F28` | （同组） | `F25` | **0.537** | 0.302 |
| `F31` | 0.724 | `F25` | **0.537** | 0.369 |
