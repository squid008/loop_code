...
2. **正交去重**（把「同一类」里多余的挑掉）：
   - 先把准入的因子**两两比一比**：比的是「**按这个因子排序，股票的先后顺序像不像**」（因子值截面秩相关）。
   - **相似度 ≥ 0.70 就算同一类**（70% 以上像，视为重复）；互相像的**归成一组**，每组只留「**剥风格 Calmar 最高**」的那一个。
   - 说白了：**像的只留最好的一个，不像的（< 0.70）全都留下** —— 这样既去重，也不会把同一组里更强的那个误删。
