...
> 当前 **12 个入库**（★ 2026-09-23 复核：`state.bank` = 12；快照权威见 `docs/loop_todo.md §0`）
...
| F09 | gen51 | 风格、财报 | ts_mean150(corr100(barra_momentum, ema20(… | 已入库(auto) |
| F10 | gen53 | 风格、财报 | ts_mean150(corr100(barra_momentum, ema20(… | 已入库(auto) |
| F11 | gen8 | 财报、风格 | max(fa_np_margin, barra_residual_volatility) | 已入库(auto) |
| F12 | gen9 | 财报、风格 | ts_mean120(max(fa_np_margin, barra_residu… | 已入库(auto) |
...
### F07 · gen55 入库（引擎自动同步，家族命名待人工精炼）
```
ts_mean150(corr100(barra_momentum, ts_mean5(fa_gm)))
```
- 符号 `sign`：**未记录**（缺失时不臆造，见 roadmap §8.45 铁律）
- 家族：风格、财报（auto）
- 叶子：barra_momentum、fa_gm
- 骨架：`ts_mean(corr(barra_momentum,ts_mean(fa_gm)))`
...
### F09 · gen51 入库（引擎自动同步，家族命名待人工精炼）
```
ts_mean150(corr100(barra_momentum, ema20(fa_sell_exp)))
```
- **符号 `sign`：`1`**（★ 因子值须乘它才是"越大越好"的方向；不乘 ⇒ 反向选股）
- 家族：风格、财报（auto）
- 叶子：barra_momentum、fa_sell_exp
- 骨架：`ts_mean(corr(barra_momentum,ema20(fa_sell_exp)))`
- 池标签：**`all3`** —— 全A **且所有池都通过**（真 alpha）
- 剥风格：**`A`** 独立有效（剥风格后**日频** Calmar >= 0.30，且**日频**回撤 > -0.20）（原 Calmar 0.776 → 剥后 0.669；超额 +5.2% → +4.3%；**日频** 剥后 Calmar 0.550，日频回撤 -7.8%）
- 口径：**20 日调仓** ✓（期数 104；成本 0.004往返(主用档)）★ 与 5 日口径的数字**不可直接比** ✗（样本区间/成本相同，只有调仓周期不同 ✓）
- 费后指标（full，成本 0.004往返(主用档)）：IC 0.0238 / IC_IR 0.237 / 年化超额 +5.2% / 回撤 -6.8% / Calmar 0.776 / Sharpe 0.729 / 最近年 +5.6% / 单期换手 23.6% / 负年 1

---


### F10 · gen53 入库（引擎自动同步，家族命名待人工精炼）
```
ts_mean150(corr100(barra_momentum, ema20(fa_gm)))
```
- **符号 `sign`：`1`**（★ 因子值须乘它才是"越大越好"的方向；不乘 ⇒ 反向选股）
- 家族：风格、财报（auto）
- 叶子：barra_momentum、fa_gm
- 骨架：`ts_mean(corr(barra_momentum,ema20(fa_gm)))`
- 池标签：**`csi1000_500_all`** —— 全A + **1000/500** 池通过
- 剥风格：**`A`** 独立有效（剥风格后**日频** Calmar >= 0.30，且**日频**回撤 > -0.20）（原 Calmar 0.782 → 剥后 0.582；超额 +5.1% → +4.3%；**日频** 剥后 Calmar 0.506，日频回撤 -8.5%）
- 口径：**20 日调仓** ✓（期数 104；成本 0.004往返(主用档)）★ 与 5 日口径的数字**不可直接比** ✗（样本区间/成本相同，只有调仓周期不同 ✓）
- 费后指标（full，成本 0.004往返(主用档)）：IC 0.0240 / IC_IR 0.242 / 年化超额 +5.1% / 回撤 -6.5% / Calmar 0.782 / Sharpe 0.716 / 最近年 +6.1% / 单期换手 23.6% / 负年 1

---


### F11 · gen8 入库（引擎自动同步，家族命名待人工精炼）
```
max(fa_np_margin, barra_residual_volatility)
```
- **符号 `sign`：`-1`**（★ 因子值须乘它才是"越大越好"的方向；不乘 ⇒ 反向选股）
- 家族：财报、风格（auto）
- 叶子：fa_np_margin、barra_residual_volatility
- 骨架：`max(fa_np_margin,barra_residual_volatility)`
- 池标签：**`csi1000_all`** —— 全A + **1000** 池通过
- 剥风格：**`A`** 独立有效（剥风格后**日频** Calmar >= 0.30，且**日频**回撤 > -0.20）（原 Calmar 0.752 → 剥后 0.358；超额 +6.3% → +2.2%；**日频** 剥后 Calmar 0.302，日频回撤 -7.5%）
- 口径：**20 日调仓** ✓（期数 104；成本 0.004往返(主用档)）★ 与 5 日口径的数字**不可直接比** ✗（样本区间/成本相同，只有调仓周期不同 ✓）
- 费后指标（full，成本 0.004往返(主用档)）：IC 0.0843 / IC_IR 0.795 / 年化超额 +6.3% / 回撤 -8.3% / Calmar 0.752 / Sharpe 0.962 / 最近年 +0.3% / 单期换手 30.6% / 负年 1

---


### F12 · gen9 入库（引擎自动同步，家族命名待人工精炼）
```
ts_mean120(max(fa_np_margin, barra_residual_volatility))
```
- **符号 `sign`：`-1`**（★ 因子值须乘它才是"越大越好"的方向；不乘 ⇒ 反向选股）
- 家族：财报、风格（auto）
- 叶子：fa_np_margin、barra_residual_volatility
- 骨架：`ts_mean(max(fa_np_margin,barra_residual_volatility))`
- 池标签：**`csi1000_500_all`** —— 全A + **1000/500** 池通过
- 剥风格：**`A`** 独立有效（剥风格后**日频** Calmar >= 0.30，且**日频**回撤 > -0.20）（原 Calmar 0.759 → 剥后 0.529；超额 +5.4% → +3.3%；**日频** 剥后 Calmar 0.420，日频回撤 -7.7%）
- 口径：**20 日调仓** ✓（期数 104；成本 0.004往返(主用档)）★ 与 5 日口径的数字**不可直接比** ✗（样本区间/成本相同，只有调仓周期不同 ✓）
- 费后指标（full，成本 0.004往返(主用档)）：IC 0.0641 / IC_IR 0.604 / 年化超额 +5.4% / 回撤 -7.1% / Calmar 0.759 / Sharpe 0.824 / 最近年 +0.0% / 单期换手 17.8% / 负年 1
