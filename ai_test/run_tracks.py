# -*- coding: utf-8 -*-
"""⚠ **兼容垫片（shim）** —— 真身在 `tools/run_tracks.py`。

存在的唯一原因：**v0.5.2 迁移时它正在跑**（PID 持有）。
该进程内部用**硬编码路径** `subprocess.run(['ai_test/run_tracks.py' ...])` 之外的调用还在，
且它自身若被再次拉起也会是 `ai_test/` 这个路径 ⇒ 直接删掉会让某次运行"文件不存在"。

**轨道结束后可安全删除本文件**（`git rm ai_test/run_tracks.py`），此后一律用 `tools/run_tracks.py`。
...
if not os.path.exists(_REAL):
    ...
    sys.exit(2)
# 让真身所在目录成为 sys.path[0]（等价于"直接跑 tools/run_tracks.py"）
sys.path.insert(0, os.path.dirname(_REAL))
sys.stdout.write('[shim] ai_test/run_tracks.py -> tools/run_tracks.py（兼容垫片，轨道结束后可删）\n')
sys.stdout.flush()
